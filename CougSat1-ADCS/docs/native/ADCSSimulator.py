...
# Iyz = 0.0
# Izz = Ixx
iBody = np.array([[Ixx, Ixy, Ixz], [Ixy, Iyy, Iyz], [Ixz, Iyz, Izz]]) / 1e6
...
def earthRadius(latitude: float) -> float:
    ...
    return np.sqrt(
        ((R1**2 * np.cos(rad)) ** 2 + (R2**2 * np.sin(rad)) ** 2)
        / ((R1 * np.cos(rad)) ** 2 + (R2 * np.sin(rad)) ** 2)
    )
...
class ADCS:
    ...
    def sensorAcquition(
        self, t: float, gps: np.ndarray, mag: np.ndarray, gravity: np.ndarray
    ) -> None:
    ...
    def compute(
        ...
        mag: np.ndarray,
        gravity: np.ndarray,
        sunDirLocal: np.ndarray,
        realTime: datetime.datetime,
        r: np.ndarray,
        omega: np.ndarray,
        ...
        if thetaError(targetGlobal, rCameraGlobal) > 0.14:
        ...
        checkOrtho = thetaError(magGlobal, targetGlobal)
        if checkOrtho > np.deg2rad(75) and checkOrtho < np.deg2rad(
            105
        ):  # and self.stage == 0:
            torque = torque2
            # print("uh oh")
...
def magnetorquer(
    N, cog: np.ndarray, iVector: np.ndarray, edge1: np.ndarray, edge2: np.ndarray
) -> tuple[np.ndarray, np.ndarray]:
    ...
    """
    rCoil = np.array([center + edge1, center - edge1, center + edge2, center + edge2]).T
...
class Satellite:
    def __init__(
        self,
        detumble: bool = False,
        initialOmega: float = 0.1,
        static: bool = False,
        sigma: float = 0.1,
        pickleFailed: bool = False,
    ) -> None:
        ...
        if detumble:
            ...
            self.geoTarget[0] = (
                self.geo[0] + np.random.uniform(-45, 45) + 180
            ) % 360 - 180
            self.geoTarget[1] = (
                self.geo[1] + np.random.uniform(-45, 45) + 90
            ) % 180 - 90
            self.geoTarget[2] = 0
            self.ecefTarget = geo2ECEF(
                self.geoTarget[0], self.geoTarget[1], self.geoTarget[2]
            )
    ...
    def run(self, progress: bool = True) -> dict:
        ...
        for i in range(1, n):
            ...
            while (
                t < tTarget and tRemaining > self.minTStep
            ):  # TODO add a keyboardinterrupt
            ...
            magnetometer = self.magFieldLocal * np.random.normal(
                1, self.sigma, size=(3, 1)
            )
            accelerometer = self.gravityLocal * np.random.normal(
                1, self.sigma, size=(3, 1)
            )
            realTime = self.startDatetime + datetime.timedelta(seconds=t)
    ...
    def __plotOrbit(self) -> None:
        ...
        ax.scatter(satX[-1], satY[-1], satZ[-1], marker="^", color="r", label="End")
        ...
        if self.geoTarget is not None:
            ecef = (
                geo2ECEF(self.geoTarget[0], self.geoTarget[1], self.geoTarget[2])
                / EARTH_A
            )
            ax.scatter(ecef[0], ecef[1], ecef[2], marker="1", color="c", label="Target")
...
def main():
    ...
    parser.add_argument(
        "-n", help="Number of monte carlo simulations to run", default=100
    )
    parser.add_argument(
        "-j",
    ...
    if args.monte_carlo:
        ...
        if results["converged"]:
            print(f'{Fore.GREEN}Converged after {results["timeToConvergence"]:8.2f}s')
        else:
            print(
