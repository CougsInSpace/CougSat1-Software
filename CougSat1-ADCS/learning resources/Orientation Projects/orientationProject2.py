...
for i in range(int(tMax/tStep)):
    ...
    axisXGlobal = r @ axisX #Don't understand this??? why rInv instead of r? Is it because of the order of matrix multiplication?
    axisYGlobal = r @ axisY
    axisZGlobal = r @ axisZ

    #Convert relevant vector to global frame
    rCameraGlobal = r @ rCamera

    #define plane of torques to get from Camera to target
    w1 = np.cross(rCameraGlobal.flatten(), targetGlobal.flatten())
    w2 = (rCameraGlobal + targetGlobal)/2
    planeNorm = np.cross(w1.flatten(), w2.flatten())
    ...
    m = 10*np.cross(planeNorm.flatten(), magGlobal.flatten()).reshape((3,1))
