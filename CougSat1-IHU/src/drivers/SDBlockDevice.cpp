...
int SDBlockDevice::_initialise_card() {
  ...
  if (_go_idle_state() != R1_IDLE_STATE) {
    DEBUG("SDCard", "No disk, or could not put SD card in to SPI idle state");
  ...
  if (SDCARD_V2 == _card_type) {
    ...
    if (BD_ERROR_OK == (status = _cmd(CMD58_READ_OCR, 0x0, 0x0, &response))) {
      ...
      if (response & OCR_HCS_CCS) {
        ...
        DEBUG("SDCard", "SDHC card initialized");
      } else {
        DEBUG("SDCard", "SD card version 2.x initialized");
      }
    }
  } else {
    _card_type = SDCARD_V1;
    DEBUG("SDCard", "SD card version 1.x initialized");
  }
...
int SDBlockDevice::init() {
  ...
  if (!_is_initialized) {
    DEBUG("SDCard", "Card initialization failed");
    unlock();
    return err;
  }
  DEBUG("SDCard", "Init card = %d", _is_initialized);
  ...
  if (_cmd(CMD16_SET_BLOCKLEN, _block_size) != 0) {
    DEBUG("SDCard", "Set %d-byte block timed out", _block_size);
    unlock();
    return BD_ERROR_DEVICE_ERROR;
  }
...
int SDBlockDevice::program(const void *b, bd_addr_t addr, bd_size_t size) {
  ...
  if (blockCnt == 1) {
    ...
    if ((response == SPI_DATA_CRC_ERROR)
        || (response == SPI_DATA_WRITE_ERROR)) {
      DEBUG("SDCard", "Single Block Write failed = 0x%x", response);
    ...
    do {
      ...
      if (response != SPI_DATA_ACCEPTED) {
        DEBUG("SDCard", "Multiple Block Write failed: 0x%x", response);
        break;
      }
      buffer += _block_size;
...
bool SDBlockDevice::_is_valid_trim(bd_addr_t addr, bd_size_t size) {
  return (addr % _erase_size == 0 && size % _erase_size == 0
      && addr + size <= this->size());
}
...
int SDBlockDevice::trim(bd_addr_t addr, bd_size_t size) {
  ...
  if (BD_ERROR_OK
      != (status = _cmd(CMD33_ERASE_WR_BLK_END_ADDR, addr + size))) {
    unlock();
    return status;
  }
...
int SDBlockDevice::_cmd(SDBlockDevice::cmdSupported cmd, uint32_t arg,
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (R1_NO_RESPONSE == response) {
      DEBUG("SDCard", "No response CMD = %d", cmd);
      continue;
    }
    break;
  }
  ...
  if (R1_NO_RESPONSE == response) {
    _deselect();
    DEBUG("SDCard", "No response CMD:%d response: 0x%x", cmd, response);
    return SD_BLOCK_DEVICE_ERROR_NO_DEVICE;         // No device
  }
  if (response & R1_COM_CRC_ERROR) {
    _deselect();
    DEBUG("SDCard", "CRC error CMD:%d response: 0x%x", cmd, response);
    return SD_BLOCK_DEVICE_ERROR_CRC;                // CRC error
  }
  if (response & R1_ILLEGAL_COMMAND) {
    _deselect();
    DEBUG("SDCard", "Illegal command CMD:%d response: 0x%x", cmd, response);
...
#ifdef DEBUG_FULL
  DEBUG("SDCard", "CMD:%d \t arg:0x%x \t Response:0x%x \n", cmd, arg, response);
#endif
  ...
  switch (cmd) {
    ...
    case CMD58_READ_OCR:                // Response R3
      ...
      response |= _spi.write(SPI_FILL_CHAR);
      DEBUG("SDCard", "R3/R7: 0x%x", response);
      break;
...
int SDBlockDevice::_cmd8() {
  ...
  if ((BD_ERROR_OK == status) && (SDCARD_V2 == _card_type)) {
    ...
    if ((response & 0xFFF) != arg) {
      DEBUG("SDCard", "CMD8 Pattern mismatch 0x%x : 0x%x", arg, response);
      _card_type = CARD_UNKNOWN;
      status = SD_BLOCK_DEVICE_ERROR_UNUSABLE;
    }
  }
...
uint8_t SDBlockDevice::_write(const uint8_t *buffer, uint8_t token,
    uint32_t length) {
  uint16_t crc = 0xFFFF;
...
bd_size_t SDBlockDevice::_sd_sectors() {
  ...
  if (_cmd(CMD9_SEND_CSD, 0x0) != 0x0) {
    DEBUG("SDCard", "Did not get a response from the disk");
    return 0;
  }
  uint8_t csd[16];
  if (_read_bytes(csd, 16) != 0) {
    DEBUG("SDCard", "Couldn't read csd response from disk");
    return 0;
  }
  ...
  switch (csd_structure) {
    case 0:
      ...
      DEBUG("SDCard", "Standard Capacity: c_size: %d", c_size);
      DEBUG("SDCard", "Sectors: 0x%x : %llu", blocks, blocks);
      DEBUG("SDCard", "Capacity: 0x%x : %llu MB", capacity,
    ...
    case 1:
      ...
      DEBUG("SDCard", "SDHC/SDXC Card: hc_c_size: %d", hc_c_size);
      DEBUG("SDCard", "Sectors: 0x%x : %llu", blocks, blocks);
      DEBUG("SDCard", "Capacity: %llu MB", (blocks / (2048U)));
      // ERASE_BLK_EN is fixed to 1, which means host can erase one or multiple of 512 bytes.
      _erase_size = BLOCK_SIZE_HC;
      break;

    default:
      DEBUG("SDCard", "CSD struct unsupported");
      return 0;
  };
  return blocks;
}
...
bool SDBlockDevice::_wait_token(uint8_t token) {
  ...
  _spi_timer.stop();
  DEBUG("SDCard", "_wait_token: timeout");
  return false;
}
