...
public:
  ...
  uint8_t readVoltage(
      LTC2499::ADCChannel_t channel, double * data, bool blocking = true);
  uint8_t readVoltageSelectNext(
      double * data, LTC2499::ADCChannel_t nextChannel, bool blocking = true);
  uint8_t readInternalTemperaure(double * data, bool blocking = true);

  uint8_t readNextActiveChannel(bool blocking = true);
  void    addActiveChannel(LTC2499::ADCChannel_t channel);
  uint8_t selectChannel(
      LTC2499::ADCChannel_t channel, bool blocking = true, bool repeated = false);

  void    setVRef(double refVoltage, double gain);
  uint8_t setVRef(double refVoltage, double gain, LTC2499::ADCChannel_t channel);
  double  getVRef();

  double getVoltage(LTC2499::ADCChannel_t channel);

private:
  uint8_t readRaw(int32_t * data, bool blocking = true);

  uint8_t configureChannel(LTC2499::ADCChannel_t channel, uint8_t config,
      bool blocking = true, bool repeated = false);

  void setVoltage(LTC2499::ADCChannel_t channel, double voltage);
  ...
  LTC2499::ADCChannel_t configuredChannel;

  std::vector<LTC2499::ADCChannel_t> activeChannels;
