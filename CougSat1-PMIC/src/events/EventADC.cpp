...
uint8_t eventADC() {
  ...
  if(result != ERROR_SUCCESS){
    ERROR("Event-ADC", "Failed to read next active channel of EPS0: 0x%02X", result);
    return result;
  }
  
  result = adcEPS1.readNextActiveChannel();
  if(result != ERROR_SUCCESS){
    ERROR("Event-ADC", "Failed to read next active channel of EPS1: 0x%02X", result);
    return result;
  }

  result = adcEPS2.readNextActiveChannel();
  if(result != ERROR_SUCCESS){
    ERROR("Event-ADC", "Failed to read next active channel of EPS2: 0x%02X", result);
    return result;
  }
  
  result = adcEPS3.readNextActiveChannel();
  if(result != ERROR_SUCCESS){
    ERROR("Event-ADC", "Failed to read next active channel of EPS3: 0x%02X", result);
    return result;
  }
  
  result = adcEPS4.readNextActiveChannel();
  if(result != ERROR_SUCCESS){
    ERROR("Event-ADC", "Failed to read next active channel of EPS4: 0x%02X", result);
    return result;
  }
  
  result = adcEPS5.readNextActiveChannel();
  if(result != ERROR_SUCCESS){
    ERROR("Event-ADC", "Failed to read next active channel of EPS5: 0x%02X", result);
    return result;
  }
